...
func TestCancelledPairingAndRecoverDoNotWrite(t *testing.T) {
	svc := newTestService(t)
	m := mustManuscript(t, svc)
	left := mustValidLeaf(t, svc, m, 1, 1, 90)
	right := mustValidLeaf(t, svc, m, 2, 1, 90)
	a := mustWatermark(t, svc, left, "A-L", "MP-001", model.WatermarkLeftHalf, 30)
	b := mustWatermark(t, svc, right, "A-R", "MP-001", model.WatermarkRightHalf, 130)

	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := svc.RequestPairing(ctx, m, a.ID, b.ID); err == nil {
		t.Fatal("cancelled RequestPairing must fail")
	}
	pairings, err := svc.ListPairings(m.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(pairings) != 0 {
		t.Fatalf("cancelled pairing must not persist, got %d", len(pairings))
	}

	if _, err := svc.Recover(ctx); err == nil {
		t.Fatal("cancelled Recover must fail")
	}
	pairings, err = svc.ListPairings(m.ID)
	if err != nil {
		t.Fatal(err)
	...
	if len(pairings) != 0 {
		t.Fatalf("cancelled Recover must not create pairings, got %d", len(pairings))
	}
}
