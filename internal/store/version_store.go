...
func (s *Store) SaveVersion(v *model.CollationVersion) error {
	...
	return s.WithTx(func(tx *sql.Tx) error {
		...
		if model.VersionStatus(curStatus).IsFrozen() && v.Status == model.VersionFrozen {
			return model.NewDomainError(model.ErrFrozen, "冻结版本 %s 的快照不可改写", v.ID)
		}
		if !model.VersionStatus(curStatus).CanTransitionTo(v.Status) {
			return model.StateTransition("校勘版本", curStatus, string(v.Status))
		}
		if model.VersionStatus(curStatus).IsFrozen() {
			// 仅允许 frozen→superseded；历史 content_json 必须原样保留。
			v.ContentJSON = storedContent
		}
		switch v.Status {
